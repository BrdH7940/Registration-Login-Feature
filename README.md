...
- API Documentation: http://localhost:3000/api (if Swagger installed)
...
*Note: Swagger is automatically disabled in production mode.*

## 🧪 Testing Security

Test validation layers work even when bypassing frontend:
...
# Mass assignment attempt - should return 400
curl -X POST http://localhost:3000/user/register \
  -H "Content-Type: application/json" \
  -d '{"email": "test@example.com", "password": "password123", "admin": true}'
```
...
This project is part of an academic assignment (IA06).
